...
victims = pd.read_csv('crime/20_Victims_of_rape.csv')
police_hr = pd.read_csv('crime/29_Period_of_trials_by_courts.csv')
auto_theft = pd.read_csv('crime/30_Auto_theft.csv')
prop_theft = pd.read_csv('crime/10_Property_stolen_and_recovered.csv')
...
for item in my_list:
  if item in input.lower():
    if item == 'rape'or item == 'harassment' :
        ...
        shp_gdf = gpd.read_file('G:\CRIMEANALYSIS\CRIMEANALYSIS\map\India States\Indian_states.shp')
        ...
        shp_gdf = gpd.read_file('map/India States/Indian_states.shp')
